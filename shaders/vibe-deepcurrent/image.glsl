...
const vec3 COLOR_SHADOW = vec3(0.02, 0.059, 0.071);

// Secondary tint mixed into the mid band gradient
const vec3 COLOR_MID = vec3(0.078, 0.471, 0.549);

// Highlight accent applied during the pulse
const vec3 COLOR_HILITE = vec3(0.471, 1, 0.863);

// Blend ratio between base and highlight colours
const float BLOOM_BLEND = 0.5;

// Angular band count around the centre
const float STRIPE_FREQUENCY = 3.8;

// Controls how sharp the angular bands appear
const float STRIPE_CURVE = 1.7;
...
const float PULSE_DEPTH = 13;

// How tightly the pulse hugs the centre
const float RADIAL_FOCUS = 3.4;

// Exponent shaping pulse falloff
const float PULSE_CURVE = 2.5;

// Saturation mix against luminance
const float SATURATION_WEIGHT = 1.05;

// Contrast multiplier around mid tones
const float CONTRAST_GAIN = 1.15;

// Post-contrast gamma curve
const float GAMMA_CURVE = 1.05;

// Vignette radius factor
const float VIGNETTE_RADIUS = 2.2;

// Controls softness of the vignette
const float VIGNETTE_POWER = 2.8;

// Blend amount for vignette application
const float VIGNETTE_MIX = 0.68;

// Strength of the animated grain
const float GRAIN_AMOUNT = 0.009;
// ---------------------------------------------------------------------------
