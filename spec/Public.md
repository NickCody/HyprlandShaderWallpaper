...
- [x] Publish `wallshader` via Cargo install
...
Pin the Rust toolchain (`rust-toolchain.toml`), list system dependencies (Wayland dev packages, GPU drivers) in `README.md`, and document the quick start commands (`git clone`, `cargo build --release`, `cargo run -p wallshader -- --help`). Add a `justfile`/`Makefile` with `setup`, `check`, and demo targets so newcomers have reproducible entry points. Tag releases and maintain a `CHANGELOG.md` using Keep a Changelog plus semantic versioning for downstream consumers.

## Publish `wallshader` via Cargo install
Document the `cargo install --git https://github.com/NickCody/WallShader --tag v0.9.1 --locked wallshader` path (or `--branch main` while iterating) and verify it builds from a clean checkout. When ready for crates.io, polish metadata and publish tagged versions so users can install without the `--git` flag.
...
1. Introduce a `paths` module (e.g. `crates/wallshader/src/paths.rs`) that wraps `directories_next::ProjectDirs` and exposes:
   - `config_dir` (`$XDG_CONFIG_HOME/wallshader`, default `~/.config/wallshader`).
   - `data_dir` (`$XDG_DATA_HOME/wallshader`, default `~/.local/share/wallshader`).
   - `cache_dir` (`$XDG_CACHE_HOME/wallshader`, default `~/.cache/wallshader`).
   - `system_share_dir` (`/usr/share/wallshader` on Unix, overridable via env/build settings).
...
3. Support env-var overrides (`WALLSHADER_CONFIG_DIR`, `WALLSHADER_DATA_DIR`, `WALLSHADER_CACHE_DIR`, `WALLSHADER_SHARE_DIR`) with documented precedence (CLI flag > env > XDG default).
4. Update config and asset loaders to rely on the module instead of ad-hoc path handling.

Implemented in commit-in-progress: `AppPaths` now resolves XDG directories (config, data, cache) plus `/usr/share/wallshader`, supports `WALLSHADER_*` overrides, and the wallpaper/multi entry points use the computed shader/cache roots.
...
1. Treat `/usr/share/wallshader` as the authoritative source for bundled shader packs, manifests, and docs.
2. Copy only missing defaults into user directories during first run or when explicitly requested.
3. Track a monotonically increasing `defaults_version` (e.g. file in `/usr/share/wallshader/VERSION`). Compare against `state.toml` and prompt for sync when upstream content is newer.
4. Provide a dry-run option that reports pending copies without making changes.

Implemented via `defaults::sync_defaults`: it scans the configured share directory, copies any missing shader packs or playlists into `~/.local/share/wallshader`, respects a `VERSION` stamp for logging, records the last sync timestamp, and updates `state.toml` only when content is actually installed.

### Phase 4 — CLI Surface & Commands *(complete)*
1. Add `wallshader defaults` subcommands:
...
Implemented: the CLI exposes `wallshader defaults where` for a quick directory overview, sharing the same path discovery logic used by the daemon.
...
1. Define resolution rules used across the app:
   ...
   - Relative paths search the process working directory, user roots, then `/usr/share/wallshader`.
...
2. Document workflows for copying defaults (`scripts/install.sh --data-dir ~/.local/share/wallshader`).
...
Implemented: docs now spell out the XDG directories, the simplified installer workflow, and packaging responsibilities for `/usr/share/wallshader`. Telemetry emits `debug!` traces for path expansion and local pack resolution, plus warnings when packs are missing or malformed so support can diagnose failures quickly.
...
Implemented via new unit and integration coverage: installer tests build temp trees, verify copies, and ensure environment overrides behave as expected. Additional `paths` tests assert env overrides and `/usr/share/wallshader` defaults (unix) so cross-platform logic stays honest.
...
Implemented: `scripts/install.sh` clones the repository (or reuses a local checkout), optionally runs `cargo install`, and flattens `shaders/` into the configured data directory (packs become `$DATA_DIR/<pack>/`, playlists land at `$DATA_DIR/<name>.toml`). The script defaults to rootless installs under `~/.local/share/wallshader`, supports `--data-dir`/`--prefix`, and exposes `--system`, `--skip-build`, and `--offline` modes for packagers. An installer integration test (`install_script_copies_defaults`) now exercises the script during `cargo test -p wallshader`, providing the CI guardrail. Packaging guidance in `README.md`/`AGENTS.md` instructs downstreams to reuse the script and include the generated assets in release bundles.
