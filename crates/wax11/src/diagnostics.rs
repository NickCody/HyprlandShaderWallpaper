...
fn dump_global_options(args: &RunArgs, paths: &AppPaths) {
    ...
    info!(
        "  still-time: {}",
        args.still_time.as_deref().unwrap_or("default")
    );
    ...
    info!(
        "  fill-method: {}",
        format_fill_method(args.fill_method.as_ref())
    );
    info!("  fps-adaptive: {}", args.fps_adaptive);
    info!(
        "  max-fps-occluded: {}",
        args.max_fps_occluded.unwrap_or(0.0)
    );
    ...
    info!(
        "  shader-compiler: {}",
        format_shader_compiler(&args.shader_compiler)
    );
    ...
    info!(
        "  shadertoy-cache: {}",
        paths.shadertoy_cache_dir().display()
    );
            ...
            if path.is_absolute() {
                ...
                for root in paths.shader_roots() {
                    ...
                    if candidate.exists() {
                        info!(
                            "  full-path: {}",
                            candidate.canonicalize().unwrap_or(candidate).display()
                        );
                        break;
                    }
                }
    ...
    if let Ok(source) = repo.resolve(&shader_handle, client, args.refresh) {
        match source {
            ShaderSource::Local(pack) => {
                ...
                info!(
                    "  shader-name: {}",
                    pack.manifest().name.as_deref().unwrap_or("unnamed")
                );
            }
            ShaderSource::CachedRemote(remote) => {
                ...
                info!(
                    "  shader-name: {}",
                    remote.pack.manifest().name.as_deref().unwrap_or("unnamed")
                );
...
fn dump_playlist_config(
    ...
    info!(
        "  workspace-switch-crossfade: {}ms",
        config.workspace_switch_crossfade().as_millis()
    );
    ...
    for (name, playlist) in &config.playlists {
        ...
        for (i, item) in playlist.items.iter().enumerate() {
            ...
            if let Ok(entry_handle) =
                EntryHandle::parse_with_resolver_or_local(resolver, &item.handle)
            {
            ...
            if let Some(antialias) = item.antialias {
                info!(
                    "      [{}] antialias: {}",
                    i,
                    format_multiconfig_antialias(&antialias)
                );
            }
...
fn format_fill_method(fill_method: Option<&FillMethod>) -> String {
    match fill_method {
        ...
        Some(FillMethod::Center {
            content_width,
            content_height,
        }) => format!("center:{}x{}", content_width, content_height),
        Some(FillMethod::Tile { repeat_x, repeat_y }) => format!("tile:{}x{}", repeat_x, repeat_y),
...
fn format_antialiasing(antialias: &Antialiasing) -> String {
    match resolve_antialiasing_info(antialias) {
        Ok(info) => format!(
            "{} ({}x, max {}x)",
            info.user_specified, info.actual_used, info.max_supported
        ),
