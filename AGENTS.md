# wax11 shader Overview

wax11 shader (wax11) is a Rust-based wallpaper engine focused on Wayland compositors. It renders GPU shaders as live backgrounds while offering deep compatibility with ShaderToy content and user-supplied shader packs.
...
- `crates/wax11`: Wayland-facing daemon. Handles configuration, CLI flags, ShaderToy client setup, and orchestrates rendering.
...
- **Path handling**: `shader://<pack>` and `playlist://<name>` handles honour `~` and shell-style `$VAR`/`${VAR}` expansions, then search the data dir, legacy `shaders/` trees, and `/usr/share/wax11`. Anything containing a `/` is interpreted literally after expansion. Missing variables fail fast so misconfigurations surface immediately.
- **Installer script**: `scripts/install.sh` (curlable via GitHub) performs a user-mode install by default, copying bundled shaders into `~/.local/share/wax11/` (packs become subdirectories, playlists flatten to top-level `.toml`). Use `--system` for `/usr/local` + `/usr/share/wax11`, or pass `--data-dir`/`--prefix` to target custom locations.
- **CLI-driven daemon**: `wax11` accepts handles like `shadertoy://ID` or local paths, supports cache-only/refresh switches, a `--shadertoy <url>` convenience flag, and `--window` testing mode.
...
- Debug mode currently replaces shader output with a full-screen pulse driven by `iTime`; wrapped GLSL dumps to `/tmp/wax11_wrapped.frag` for inspection each compile.
...
- Wall-clock diagnostics now use the `[wax11]` prefix instead of `[wax11]`; tracing output goes
  through `scripts/launch-local`.
- Tests covering workspace crossfades and failure handling live in `crates/wax11/src/multi.rs` (`workspace_switch_applies_crossfade_override`
...
- Run `cargo run -p wax11 -- --shadertoy https://www.shadertoy.com/view/3dXyWj --window`, then inspect `/tmp/wax11_wrapped.frag` to ensure no leftover `uniform iTime`/`iChannel*` lines remain and macros look correct.
...
- User directories: config `~/.config/wax11`, data `~/.local/share/wax11`, cache `~/.cache/wax11`. Override with `WAX11_CONFIG_DIR`, `WAX11_DATA_DIR`, `WAX11_CACHE_DIR`, `WAX11_SHARE_DIR`.
...
- **To customize shaders:** Copy from DATA_DIR or SHARE_DIR to CONFIG_DIR, then edit. wax11 shader will prefer the CONFIG_DIR version.
- **AppImage first-run:** On startup, bundled assets from SHARE_DIR (`$APPDIR/opt/wax11`) are automatically extracted to DATA_DIR once per version, always overwriting. CONFIG_DIR is never touched by the extractor.
- Inspect paths with `wax11 defaults where` to confirm the active layout.
...
**Problem:** wax11 shader was causing Firefox to freeze when rendering ShaderToy WebGL content, due to aggressive GPU resource claims.
...
**Impact:** These changes make wax11 shader a better "background citizen" that won't starve foreground GPU applications. The visual quality and animation smoothness should remain unchanged since wallpapers don't need aggressive performance optimization.
...
```bash
wax11 --gpu-power high --gpu-memory performance --gpu-latency 1
```

**Implementation:**
- Added types to `renderer/src/types.rs`: `GpuPowerPreference`, `GpuMemoryMode`
- Added CLI types and parsers to `wax11/src/cli.rs`
